...
function renderProjects(projects, lang, containerId = 'projects-container', limit = null) {
  ...
  const defaultProjectSvg = `data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="400" height="220" viewBox="0 0 400 220"><rect width="400" height="220" fill="%23111111"/><path d="M0 0 L400 220 M400 0 L0 220" stroke="%23c5a059" stroke-width="0.5" stroke-opacity="0.1"/><circle cx="200" cy="110" r="45" fill="none" stroke="%23c5a059" stroke-width="1" stroke-opacity="0.15"/><text x="50%" y="50%" font-family="sans-serif" font-size="12" fill="%23c5a059" letter-spacing="2" font-weight="bold" fill-opacity="0.4" dy=".3em" text-anchor="middle">PROJECT</text></svg>`;
  ...
  projectsToRender.forEach((proj, index) => {
    ...
    const imageSrc = proj.image || defaultProjectSvg;
    ...
    card.innerHTML = `
      <div class="img-zoom-container project-img-wrapper">
        <img src="${imageSrc}" alt="${title}" class="project-img" onerror="this.src='${defaultProjectSvg}'">
      </div>
  ...
  experience.forEach((exp, index) => {
    ...
    const displayAction = (exp.certificate || exp.image) ? `
      <div class="project-actions" style="margin-top: auto;">
        <button class="btn-primary" onclick="window.openCertModal('${exp.id}')" style="width: 100%; font-size: 0.85rem; padding: 0.55rem 1rem;">${buttonText}</button>
      </div>
    ` : '';
...
window.openProjectModal = function(id) {
  ...
  document.getElementById('modal-img').src = proj.image || defaultProjectSvg;
...
window.openCertModal = function(id) {
  const exp = currentExperience.find(e => e.id === id);
  if (!exp) return;
  
  const lang = document.documentElement.lang || 'en';
  const title = lang === 'ar' && exp.titleAr ? exp.titleAr : exp.title;
  const certPath = exp.certificate || exp.image || '';
  ...
  const errorEl = document.getElementById('cert-modal-error');
  const errorPathEl = document.getElementById('cert-modal-error-path');
  const downloadEl = document.getElementById('cert-modal-download');
  
  if (certPath) {
    downloadEl.href = certPath;
    ...
    if (isPdf) {
      ...
      if (pdfContainer) {
        pdfContainer.innerHTML = `<iframe src="${certPath}" style="width: 100%; height: 50vh; border: none; border-radius: 6px;"></iframe>`;
    ...
    errorPathEl.textContent = lang === 'ar' ? 'لم يتم تحديد مسار للمستند في ملف JSON' : 'No document path specified in JSON';
    downloadEl.style.display = 'none';
  }
  
  if (errorPathEl && certPath) {
    errorPathEl.textContent = `${lang === 'ar' ? 'المسار المطلوب:' : 'Expected path:'} ${certPath}`;
  }
